...
export class AliasTableComponent
{
    ...
    public definition: TableDefintion<AliasRecord> = {
        toggleUpdate: false,
        name: 'AliasTable',
		helpTopic: 'alias-table',
		defaultSortField: 'IA_ID',
        ...
        rowDoubleClick: (core: any, self: any, idx: number, row: AliasRecord) => {
		...
		columns: [
            {
				...
				filter: false,
				sort: false,
                cell: (row: AliasRecord) => row.IA_NAME
            },
            {
                columnDef: 'IA_ALIAS',
				header: "Alias",
				...
				filter: false,
				sort: false,
                cell: (row: AliasRecord) => row.IA_ALIAS
            },
            {
                columnDef: 'IA_INDEX',
				header: "Index",
				...
				filter: false,
				sort: false,
                cell: (row: AliasRecord) => row.IA_INDEX
            },
            {
                columnDef: null,
				display: true,
				header: 'Options',
				width: '70px',
				cell: (row: AliasRecord) => {},
                buttons: [
                    {
						label: 'Delete',
						icon: 'delete',
						action: (core: any, self: any, idx: number, row: AliasRecord) => {
							core.deleteRecord( idx, row, 'IA_ID', 'Alias', 'IA_NAME'  );
						}
					},
                ]
            }
    ...
    constructor( dataService: AliasDataService
    ...
    public addRecord(): void
	{
        ...
        this.router.navigate( ['/alias/edit'], {
			queryParams: { mode: 'new' }
		} );
		return;
	}

    public editRecord( idx: number, row: AliasRecord ): void
	{
        this.router.navigate( ['/alias/edit'], {
			queryParams: { 	id: 'IA_ID', mode: 'edit', value: row.IA_ID }
		} );
        return;
	}
}
